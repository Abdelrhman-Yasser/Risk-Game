...
/* constructor */
/******************************************/
map_file_parser::map_file_parser(char *init_file_dir)
{
	// read data from file
	this->buffered_file = vector<string>();
	io_handler::read_file(init_file_dir, &buffered_file);
...
/******************************************/
int
map_file_parser::parse_count(int line_number)
{
	/*
		gets count of countries, borders, and continents
		simple specify attributes and line in which count is specified
	*/
	string line = this->buffered_file.at(line_number); // gets specified line
	vector<string> line_token;
	tokenize(line, &line_token); // split by delimiter
...
/******************************************/
void
map_file_parser::tokenize(string target_line, vector<string> *tokens)
{
...
/******************************************/
int
map_file_parser::get_country_count()
{
	return this->country_count;
}

int
map_file_parser::get_border_count()
{
	return this->border_count;
}

int
map_file_parser::get_continent_count()
{
...
/******************************************/
void
map_file_parser::obtain_border_list(vector<struct border> *border_list)
{
	int index = 2;
	...
	{
		// tokenize
		string line = this->buffered_file.at(index); // gets specified line
		vector<string> line_token;
		tokenize(line, &line_token);
...
/******************************************/
void
map_file_parser::obtain_continent_list(vector<struct continent> *continent_list)
{
	...
	while(count > 0)
	{
		...
		string line = this->buffered_file.at(index); // gets specified line
		vector<string> line_token; 
		tokenize(line, &line_token); // tokenize
